"""U-Net 推理与评估：计算 Dice、HD95，并保存预测结果。
...
def load_model(checkpoint_path: Path, model_cfg: dict, device: torch.device) -> UNet:
    model = UNet(
        in_channels=1,
...
def update_class_metrics(
    ...
    metrics.setdefault("dice", []).append(dice)
    if np.isfinite(hd95):
        metrics.setdefault("hd95", []).append(hd95)
...
def main():
    ...
    checkpoint_path = ROOT / test_cfg.get(
        "checkpoint",
        str(EXP_DIR.relative_to(ROOT) / "outputs" / "best_unet.pth"),
    )
